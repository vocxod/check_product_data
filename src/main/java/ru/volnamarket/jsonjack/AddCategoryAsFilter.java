 ...
 *
 */
public class AddCategoryAsFilter {
                ...
                for (int i = 0; i < iLast - 1; i++) {
                    ...
                    for (File jsonProductFile : jsonProductFiles) {
                        ...
                        for (FullAttribute item : aFullAttributes) {
                            ...
                            // create filter for category and product by ALL LINK site data item
                            if (item.isIs_link()) {
                                // Это наш фильтр (в терминах опенкарта, или "срез" по нашему)
                                // System.out.println(item);
                                UpsertFilter(item, productTwo);
                            }
                            // only CATEGORY or MANUFACTURER
                            /*
    ...
    public static int UpsertFilter(FullAttribute item, Product product) {
                ...
                "volna", "bBD65855ZLzl@@@###");) 
        {
            ...
            if (item.getValue().length() < 1) {
              System.out.println(ANSI_RED + "PID:" + product + "FILTER" 
                  + item.getValue() + ANSI_RESET);
              System.exit(0);
              return 0;
            }
            ...
            ResultSet resultSet;
             ...
             /* @TODO
             * Need fix - we skip product if ptoduct id exist in oc_product_filter table
            */
            String sSqlSkip = "SELECT product_id FROM oc_product_filter WHERE product_id=" 
              + product.getProduct_id();
            resultSet = statement.executeQuery(sSqlSkip);
            if (resultSet.next()) {
              // System.out.print(ANSI_RED + "." + ANSI_RESET);
              // System.exit(0);
              return 0; // skip exit product
            }
